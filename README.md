...
### 10. CARSI 登录中间可能卡在学校 IdP 的"信息发布确认页"

首次使用 CARSI 访问某个出版商时，学校 IdP 会弹出确认页（合工大显示"信息发布"，询问"是否同意共享这些信息给 Elsevier 等服务"）。此时：
- 在页面上点击「接受」（或"我同意自动发送相同的信息到此服务"）即可完成授权
- 脚本已能识别该页面并打印提示；授权一次后，后续访问通常不再询问
- 授权后会自动跳回出版商，脚本检测到登录成功并继续下载
